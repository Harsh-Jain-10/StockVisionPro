...
def ai_screener(db: Session) -> dict[str, Any]:
    """Fetch technical data for NSE universe and analyze for bullish candidates."""
    stock_data = []
    for symbol in NSE_UNIVERSE:
        try:
            quote = get_quote(symbol, db)
            # Compute RSI and MACD using yfinance + ta directly
            try:
                import yfinance as _yf
                import ta as _ta
                _df = _yf.Ticker(symbol).history(period="3mo")
                rsi = float(_ta.momentum.rsi(_df["Close"], window=14).iloc[-1]) if len(_df) > 14 else None
                _macd_obj = _ta.trend.MACD(_df["Close"])
                macd = float(_macd_obj.macd().iloc[-1]) if len(_df) > 26 else None
                macd_signal = float(_macd_obj.macd_signal().iloc[-1]) if len(_df) > 26 else None
            except Exception:
                rsi = None
                macd = None
                macd_signal = None
    ...
    # Score each stock for bullishness
    scored = []
    for s in stock_data:
        score = 0
        ...
        if s.get("rsi") and 30 < s["rsi"] < 65:
            score += 1  # RSI in healthy range (not overbought)
    ...
    # Build AI analysis text without external AI API (uses rule-based reasoning)
    analysis_parts = []
    for i, s in enumerate(top_bullish, 1):
        reasons = []
        if s.get("change_pct") and s["change_pct"] > 0:
            reasons.append(f"up {s['change_pct']:.2f}% today")
        if s.get("rsi"):
            if s["rsi"] < 50:
                reasons.append(f"RSI at {s['rsi']:.1f} (room to run)")
            elif s["rsi"] < 65:
                reasons.append(f"RSI at {s['rsi']:.1f} (healthy momentum)")
        if s.get("macd") and s.get("macd_signal") and s["macd"] > s["macd_signal"]:
            reasons.append("MACD above signal line (bullish crossover)")

        reason_str = ", ".join(reasons) if reasons else "showing relative strength in current market"
        analysis_parts.append(
            f"{i}. **{s['symbol']}** — {reason_str}. "
            f"Current price: {s['price']:.2f if s['price'] else 'N/A'}."
        )

    analysis_text = "\n".join(analysis_parts)
    if not analysis_text:
        analysis_text = "No strongly bullish signals detected in the NSE universe at this time. Consider waiting for clearer setups."
